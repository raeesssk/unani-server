...
router.post('/add', oauth.authorise(), (req, res, next) => {
  ...
  pool.connect(function(err, client, done){
    ...
    client.query('INSERT INTO category_master(ctm_type, ctm_status) values($1,0)',[req.body.ctm_type]);
    // SQL Query > Select Data
    const query = client.query('SELECT * FROM category_master');
    query.on('row', (row) => {
      results.push(row);
    });
    query.on('end', () => {
...
router.post('/edit/:ctmId', oauth.authorise(), (req, res, next) => {
  ...
  pool.connect(function(err, client, done){
    ...
    // SQL Query > Insert Data
    client.query('UPDATE category_master SET ctm_type=$1 where ctm_id=$2',[req.body.ctm_type,id]);
    // SQL Query > Select Data
    const query = client.query('SELECT * FROM category_master');
    query.on('row', (row) => {
      results.push(row);
...
router.post('/delete/:ctmId', oauth.authorise(), (req, res, next) => {
  ...
  pool.connect(function(err, client, done){
    ...
    client.query('UPDATE category_master SET ctm_status=1 WHERE ctm_id=($1)', [id]);
    // SQL Query > Insert Data
